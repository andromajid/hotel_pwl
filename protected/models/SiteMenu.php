...
class SiteMenu extends CActiveRecord {
    ...
    public function getMenu($type = 'admin', $menu_par_id = '0') {
        $result = Yii::app()->db->createCommand()->select('*')->from('site_menu')->
                        where('menu_location=:menu AND menu_par_id=:par_id', array(':menu' => $type, ':par_id' => $menu_par_id))->order('menu_order_by ASC')->queryAll();
        return $result;
    }
    ...
    public function getAllMenu($type = 'admin') {
        $result = Yii::app()->db->createCommand()->select('*')->from('site_menu')->
                        where('menu_location=:menu', array(':menu' => $type))->order('menu_order_by ASC')->queryAll();
        return $result;
    }
    ...
    /** 
     * fungsi buat update data ke bawah atau ke atas
     * @param String $condition >= atau <=
     * @param String $order_by order by sql
     * @param int $menu_id primary dari site_menu
     * @param String $sort sorting sql
     * @param Int $menu_par_id bapak dari menu tersebut
     * @param String $menu_location Lokasi menu-nya
     */
    public function getMenuForUpdate($condition, $order_by, $menu_id, $sort, $menu_par_id = 0, $menu_location = 'admin') {
       return Yii::app()->db->createCommand()->select('menu_id, menu_order_by')->from('site_menu')->where('menu_order_by '.$condition.' :menu_order_by AND menu_id != :menu_id
                                                AND menu_location = :menu_location AND menu_par_id=:menu_par_id', 
                                                array(':menu_id' => $menu_id,
                                                      ':menu_order_by' => $order_by,
                                                      ':menu_id' => $menu_id,
                                                      ':menu_location' => $menu_location,
                                                      ':menu_par_id' => $menu_par_id))->order('menu_order_by '.$sort)
                                                ->queryRow();
    }
}
